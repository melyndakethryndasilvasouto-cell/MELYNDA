...
export default function GroupChatPage() {
  ...
  const { safetyAccepted, status, userId, players, acceptSafety, connect, inviteToGroup, blockPlayer, reportPlayer } = useOnline()
  ...
  const inviteDialogFirstRef = useRef<HTMLButtonElement>(null)
  const closeInviteDialog = useCallback(() => setInviteOpen(false), [])
  useAccessibleDialog(inviteOpen, closeInviteDialog, inviteDialogFirstRef)
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`online-group-events:${groupId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'online_group_messages', filter: `group_id=eq.${groupId}` }, () => { if (active) void load().catch(() => { setGroup(null); setError('Este grupo foi encerrado ou não está mais disponível.') }) })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'online_group_members', filter: `group_id=eq.${groupId}` }, () => { if (active) void load().catch(() => { setGroup(null); setError('Você não participa mais deste grupo.') }) })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'online_groups', filter: `id=eq.${groupId}` }, () => { if (active) void load().catch(() => { setGroup(null); setError('Este grupo foi encerrado.') }) })
      .subscribe()
  ...
  if (status === 'error') return <section className="glass-card mx-auto mt-8 max-w-md p-6 text-center"><h1 className="font-title text-2xl" style={{ color: '#5B3A8A' }}>O grupo perdeu a conexão</h1><p role="alert" className="mt-2 text-sm">Tente entrar novamente. Sua mensagem não será enviada duas vezes.</p><div className="mt-4 grid gap-2"><button type="button" className="btn-primary" onClick={() => void connect()}>Tentar novamente</button><button type="button" className="btn-secondary" onClick={() => navigate('/online')}>Voltar ao Online</button></div></section>
  ...
  return (
    <section className="pb-10 pt-3">
      <header className="glass-card p-4">
        ...
        <div className="mt-2 flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><h1 className="break-words font-title text-3xl" style={{ color: '#5B3A8A' }}>👥 {group.name}</h1><p className="text-sm" style={{ color: '#4B5563' }}>{members.length} de {group.max_members} participantes</p></div><div className="flex gap-2">{owner && <button type="button" className="btn-primary min-h-11 px-3 text-sm" onClick={() => { setError(''); setInviteOpen(true) }}><Users size={17} /> Convidar</button>}<button type="button" className="btn-secondary min-h-11 px-3 text-sm" onClick={() => void leaveOrClose()}>{owner ? <Trash2 size={17} /> : <LogOut size={17} />}{owner ? 'Encerrar' : 'Sair'}</button></div></div>
      </header>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_250px]">
        <section className="glass-card flex min-h-[620px] min-w-0 flex-col overflow-hidden" aria-labelledby="group-chat-title">
          ...
          <div ref={messageLogRef} onScroll={event => { const log = event.currentTarget; keepAtBottomRef.current = log.scrollHeight - log.scrollTop - log.clientHeight < 80; if (keepAtBottomRef.current) setNewMessages(false) }} className="relative min-h-52 flex-1 space-y-3 overflow-y-auto bg-slate-50 p-3" role="log" aria-live="polite" aria-label="Mensagens do grupo">{messages.length === 0 ? <p className="py-10 text-center text-sm" style={{ color: '#6B7280' }}>Escreva uma mensagem gentil para a turma.</p> : messages.map(message => { const mine = message.sender_id === userId; return <article key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}><div className="max-w-[88%] rounded-2xl px-3 py-2" style={{ background: mine ? '#DBEAFE' : '#F3E8FF', overflowWrap: 'anywhere' }}><strong className="block text-xs">{message.sender_avatar} {message.sender_name}</strong>{message.kind === 'text' ? <p className="mt-1 text-sm">{message.body}</p> : <audio controls preload="none" src={message.audio_data || ''} className="mt-2 max-w-full" aria-label={`Áudio de ${message.sender_name}`} />}</div></article> })}</div>
          {newMessages && <button type="button" className="mx-auto -mt-12 mb-2 min-h-11 rounded-full bg-blue-700 px-4 text-sm font-black text-white shadow-lg" onClick={scrollMessagesToEnd}>Novas mensagens ↓</button>}
        ...
        <aside className="glass-card h-fit p-4" aria-labelledby="members-title"><h2 id="members-title" className="font-black" style={{ color: '#5B3A8A' }}>Participantes</h2><div className="mt-3 space-y-3">{members.map(member => <article key={member.user_id} className="rounded-2xl bg-white p-3"><div className="flex items-center gap-2"><span className="text-2xl">{member.avatar}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm">{member.display_name}</strong><span className="text-xs">{member.role === 'owner' ? 'Dono do grupo' : 'Convidado'}</span></span>{member.role === 'owner' && <Crown size={17} style={{ color: '#B45309' }} />}</div>{member.user_id !== userId && <div className="mt-2 grid grid-cols-2 gap-1"><button type="button" className="min-h-11 rounded-xl bg-slate-100 text-xs font-bold" onClick={() => void protect(member)}><Ban className="inline" size={14} /> Bloquear</button><button type="button" className="min-h-11 rounded-xl bg-orange-50 text-xs font-bold" style={{ color: '#9A3412' }} onClick={() => void protect(member, true)}><AlertTriangle className="inline" size={14} /> Denunciar</button>{owner && member.role !== 'owner' && <button type="button" className="col-span-2 min-h-11 rounded-xl bg-red-50 text-xs font-bold" style={{ color: '#B91C1C' }} disabled={busy === member.user_id} onClick={() => void removeMember(member.user_id)}>Remover do grupo</button>}</div>}</article>)}</div></aside>
      </div>
