    ...
    <aside className="fixed inset-x-3 top-[4.5rem] z-[80] mx-auto max-w-lg rounded-3xl border-2 border-blue-200 bg-white p-4 shadow-2xl" role="dialog" aria-modal="false" aria-labelledby="online-notification-title">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {playInvite ? <><h2 id="online-notification-title" className="mt-1 font-black" style={{ color: '#374151' }}>{playInvite.from_avatar} {playInvite.from_name} chamou você para jogar.</h2>{currentGame && <p className="mt-1 text-sm" style={{ color: '#4B5563' }}>Você está em {currentGame}. Nada será fechado sem você escolher.</p>}</> : <><h2 id="online-notification-title" className="mt-1 font-black">{groupInvite.from_avatar} {groupInvite.from_name} convidou você para “{groupInvite.group_name}”.</h2><p className="mt-1 text-sm">Somente convidados podem entrar nesse grupo.</p></>}
        </div>
      </div>
