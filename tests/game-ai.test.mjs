...
test('os demais jogos preservam motores locais para o computador', async () => {
  ...
  assert.match(files[1], /aiPickMove\(board, mode\)/)
  assert.match(files[1], /'hard'/)
  ...
  assert.match(files[2], /aiChooseCard\(cur\.hand, top, s\.activeColor, difficulty\)/)
  assert.match(files[2], /type Difficulty = 'easy' \| 'medium' \| 'hard'/)
